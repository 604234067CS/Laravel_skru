...
<body>
    ...
    <form action="{{ route('article.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        ...
        {{ method_field('PUT') }}
        <label>หัวข้อ</label>
        <input type="text" class="form-control" name="title" placeholder="ระบุหัวข้อ" value="{{ $article->title }}" required>
        <br>
        ...
        <input class="form-control datepicker" type="date" name="date" placeholder="ระบุวันที่" value="{{ $article->date }}" required>
        <br>
        <label>หน้าปก</label>
        ...
        <input type="text" class="form-control" name="author" placeholder="ระบุผู้เขียน" value="{{ $article->author }}" required>
        <br>
        <label>รายละเอียด</label>
        <textarea name="details" id="details" rows="8" placeholder="ระบุรายละเอียด...">{{ $article->details }}</textarea>
        <br>
        @if (session('message'))
        ...
        @endif
        <br>
        <button type="submit" class="btn btn-primary"><i class="fas fa-check-circle fa-fw"></i> แก้ไขข้อมูล</button>
